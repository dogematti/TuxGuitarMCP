...
## 5. Roadmap v3: the arrangement layer - SHIPPED same evening
...
1. DONE - tuxguitar_generate_transitions: fill/buildup/stop, one bar or
   before every marker at once; melodic tracks ramp or stop with letRing.
2. DONE - tuxguitar_ornament: standard/aggressive/tremolo flavors,
   deterministic, realism-aware.
3. DONE - tuxguitar_plan_harmony (7 mood recipes or explicit degrees,
   voice-leading verdicts, pedal warning) + max_literal_share quota in
   every style rubric, enforced by evaluate { style }.
4. DONE - tuxguitar_generate_lead: 9 lick cells, contour arc, breathing
   rests, question/answer phrase ends, root resolution, difficulty cap
   with automatic thinning.
5. DONE - generate_riff tension_target (numeric or emotion words)
   shapes density and register per bar; phrase_len cadences resolve to
   the root.
6. DONE - pass history persisted to ~/.tuxguitar-mcp/passes/
   <documentId>.json (survives embedded-chat turns); tuxguitar_
   diff_measures; per-section groove/syncopation/rest metrics;
   structured player-note hints (cells/density/syncopation/register/
   accents/phrase "key = value" lines) feed generate_riff via style=.
